...
*/
    ...
    APPLY_SBOX_LO(0x000F000F, 0xFF00FF00, 0); // 4,0
    APPLY_SBOX_HI(0x00F000F0, 0xFF00FF00, 1); // 5,1
    APPLY_SBOX_LO(0x0F000F00, 0x00FF00FF, 2); // 6,2
    APPLY_SBOX_HI(0xF000F000, 0x00FF00FF, 3); // 7,3
    out = uncollect_w16(out);
...
*/
    ...
    Vector256 gfunc_out;
    Vector256_from_m256i(&gfunc_out, collect_w16(Vector256_to_m256i(&vv)));
    intf->printf(">>>>>>>>>>>>>>>\n");
    Vector256_print(&vv, intf);
    Vector256_print(&gfunc_out, intf);
    intf->printf(">>>>>>>>>>>>>>>\n");
    Vector256_from_m256i(&gfunc_out, uncollect_w16(Vector256_to_m256i(&gfunc_out)));
    Vector256_print(&gfunc_out, intf);    
    intf->printf("------------->>>>>>>>>>>>>>>\n");
